...
import 'package:fuodz/views/pages/taxi/give_tip.page.dart';
...
class NewTaxiOrderSummaryViewModel extends MyBaseViewModel {
  ...
  void openPaymentMethodSelection() async {
    ...
    final mPaymentMethod = await viewContext.push(
      (context) =>
          PaymentMethodSelectionPage(list: taxiViewModel.paymentMethods),
    );
  ...
  void openCoupnDialog() async {
    ...
    final result = await Navigator.of(viewContext).push(
      MaterialPageRoute(
        builder: (context) => ApplyDiscountPage(coupon: taxiViewModel.coupon),
      ),
    );
    //
  ...
  openTipDialog() async {
    final result = await Navigator.of(viewContext).push(
      MaterialPageRoute(builder: (_) => GiveTipPage(tip: taxiViewModel.tip)),
    );
    //
    if (result != null && result.toString().isNotEmpty) {
      this.taxiViewModel.tip = double.parse(result.toString());
    } else {
      this.taxiViewModel.tip = 0.00;
    }
    await this.taxiViewModel.calculateTotalAmount();
  }
}
