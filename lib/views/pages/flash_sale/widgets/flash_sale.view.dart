...
import 'package:fuodz/widgets/slide_countdown.dart';
  ...
  List<Widget> flashSalesListView(BuildContext context, FlashSaleViewModel vm) {
    ...
    flashsales.forEach((flashsale) {
      ...
      Widget title = HStack(
        [
          ...
          "SEE ALL"
              ...
              .color(Utils.textColorByColor(AppColor.closeColor))
              .make()
              .onTap(() => openFlashSaleItems(context, flashsale)),
        ],
        spacing: 10,
      ...
      Widget items = CustomListedListView(
        ...
        items:
            (flashsale.items ?? []).map((flashSaleItem) {
              return FittedBox(
      ...
      //
      list.add(VStack([title, items.py4(), UiSpacer.vSpace(10)]).py12());
    });
