...
import 'package:fuodz/widgets/states/vendor_type.empty.dart';
...
class EmptyWelcome extends StatelessWidget {
    ...
    return VStack([
      ...
      VStack([
            ...
            VStack([
              ...
              UiSpacer.vSpace(12),
              //waiting on location - show a loading placeholder instead of
              //the empty state, since an empty result here doesn't yet mean
              //"no services" (see WelcomeViewModel.isAwaitingLocation).
              //isBusy already gets its own shimmer via CustomListView below.
              if (vm.shouldShowVendorTypesLoading)
                LoadingShimmer().px20().centered(),
              //empty state (shared by list/grid views)
              if (vm.shouldShowEmptyVendorTypes)
                EmptyVendorTypes(
                  onRetry: () {
                    vm.initialise(initial: false);
                  },
                ).py20(),
              //list view
              if (_showListView())
  ...
  //MISC.
  bool _showListView() {
    return ((HomeScreenConfig.isVendorTypeListingBoth && !vm.showGrid) ||
            (!HomeScreenConfig.isVendorTypeListingBoth &&
                HomeScreenConfig.isVendorTypeListingListView)) &&
        (vm.isBusy || vm.vendorTypes.isNotEmpty);
  }

  bool _showGridView() {
    return HomeScreenConfig.isVendorTypeListingGridView &&
        vm.showGrid &&
        !vm.isBusy &&
        vm.vendorTypes.isNotEmpty;
  }
}
